...
public class ExcavateHistory
{
	...
	public BlockHistory recordPosition(World world, BlockPos pos)
	{
		...
		TileEntity tile = world.getTileEntity(pos.getX(), pos.getZ(), pos.getY());
	...
	{
		if(history.contains(block) || block.block == Blocks.air)
		{
			return;
		}
	...
	public RestoreResult canRestore(MinecraftServer server, EntityPlayer player)
	{
		...
		{
			if(world.getBlock(hist.pos.getX(), hist.pos.getY(), hist.pos.getZ()) != Blocks.air)
			{
				return RestoreResult.OBSTRUCTED;
			} else if(world.getEntitiesWithinAABBExcludingEntity(null, AxisAlignedBB.getBoundingBox(hist.pos.getX(), hist.pos.getY(), hist.pos.getZ(), hist.pos.getX() + 1F, hist.pos.getY() + 1F, hist.pos.getZ() + 1F)).size() > 0)
			{
				return RestoreResult.OBSTRUCTED;
			}
		}
		...
		if(!player.capabilities.isCreativeMode && stacks.size() > 0)
		{
			...
			while(iterStacks.hasNext())
			{
				...
				for(int i = 0; i < player.inventory.getSizeInventory() && stack.stackSize > 0; i++)
				{
					...
					if(stack.equals(invoStack))
					{
						int num = Math.min(stack.stackSize, invoStack.stackSize);
