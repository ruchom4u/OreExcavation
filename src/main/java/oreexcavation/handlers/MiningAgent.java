...
public class MiningAgent
{
		...
		for(int n = 0; !scheduled.isEmpty(); n++)
		{
			...
			if(flag)
			{
				...
				if(!(ExcavationSettings.ignoreTools || ToolEffectiveCheck.canHarvestBlock(player.worldObj, s, pos, player)))
				{
					checkedBlocks.add(pos);
					continue;
				} else if(player.interactionManager.tryHarvestBlock(pos) || player.worldObj.getBlockState(pos).getBlock() == Blocks.AIR)
				{
					if(ExcavationSettings.maxUndos > 0)
					{
	...
	public void appendBlock(BlockPos pos)
	{
		...
		{
			return;
		} else if(player.getDistance(pos.getX(), pos.getY(), pos.getZ()) > toolProps.getRange() || !player.worldObj.getWorldBorder().contains(pos) || !canDestroy(player, pos))
		{
			return;
	...
	private boolean canDestroy(EntityPlayer player, BlockPos pos)
	{
		...
		return held != null && state.getBlock() != Blocks.AIR && held.canDestroy(state.getBlock());
	}
