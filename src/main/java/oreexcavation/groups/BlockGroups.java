...
public class BlockGroups
{
	...
	public List<BlockEntry> getGroup(Block block, int metadata)
	{
		...
		for(List<BlockEntry> l2 : groupList)
		{
			for(BlockEntry e : l2)
			{
				if(e != null && e.checkMatch(block, metadata))
				{
					list.addAll(l2);
					break;
				}
	...
	public boolean isStrict(Block block, int metadata)
	{
		return quickCheck(strictSubs, block, metadata);
	}
	
	public boolean quickCheck(List<BlockEntry> list, Block block, int metadata)
	{
